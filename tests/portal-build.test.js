...
test("portal inline build optimizes screenshot assets before inlining", async () => {
  ...
  assert.match(asciiPortalHtml, /\.hero \.button--primary:is\(:link, :visited, :hover, :focus-visible, :active\) \{\s*background: var\(--tiffany-dark\) !important;\s*color: var\(--white\) !important;\s*text-decoration: none !important;\s*text-decoration-color: transparent !important;/);
