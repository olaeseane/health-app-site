...
test("header links to the five current landing sections", () => {
  ...
  assert.match(styles, /\.button--primary:hover\s*\{[^}]*background: var\(--tiffany-deep\);[^}]*box-shadow: 0 16px 34px rgba\(18, 104, 107, 0\.24\);[^}]*transform: translateY\(-1px\);/);
  assert.match(styles, /\.button--primary:active\s*\{[^}]*background: var\(--tiffany-deep\);[^}]*box-shadow: 0 10px 24px rgba\(18, 104, 107, 0\.2\);[^}]*transform: translateY\(0\);/);
  assert.match(styles, /\.button--primary:focus-visible\s*\{[^}]*outline: 3px solid rgba\(127, 211, 205, 0\.45\);[^}]*outline-offset: 3px;/);
  assert.match(
    styles,
...
test("download route follows privacy with two real QR links", () => {
